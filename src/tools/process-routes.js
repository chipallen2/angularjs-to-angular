...
module.exports = function (files, routeNames) {

    const routeNamesArray = routeNames.split(',');

    files.forEach((file, index) => {
        const routeName = routeNamesArray[index];
        console.log(`Processing: ${path.basename(file)} | Route Name: ${routeName}`);
    ...
    console.log(`Converted ${files.length} routes in ${tmpPath.replace(process.cwd() + '/', '')}.\n\n`);
};